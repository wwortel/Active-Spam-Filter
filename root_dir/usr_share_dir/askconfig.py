...
class AskConfig:
	...
	def __read_config(self, configfile):
		...
		try:
			self.cfg.read(configfile, encoding='ISO-8859-1')
		...
		if not os.path.exists(self.rc_msgdir):
			os.mkdir(self.rc_msgdir)

		if not os.stat(self.rc_msgdir).st_mode == stat.S_IRWXU | stat.S_IRWXG :
			os.chmod(self.rc_msgdir, stat.S_IRWXU | stat.S_IRWXG )
	...
	def __expand(self, option, listmode = 0, mandatory = 1, boolean = 0, default = None):
		...
		except configparser.NoSectionError as est:
			print("Config file error: %s" % est)
			sys.exit(self.RET_PROCMAIL_CONTINUE)

		except configparser.NoOptionError as est:
			if not default == None:
				return default
			if not mandatory:
				if listmode:
		...
		while 1:
			...
			buf = buf.replace(res.group(1), result)

		## List?

		if listmode:
			listbuf = buf.split(",")
			return list( map(lambda bstr: bstr.strip(), listbuf) )

		## If boolean, interpret true/false/on/off/1/0

		if boolean:
			if buf.lower() in ("true","on","1","yes"):
				return 1
			else:
				return 0
