...
class AskLog:
	...
	#------------------------------------------------------------------------------
	def write(self, level, ustr):
		"Generate a log output if self.loglevel >= level and outfile is not null"

		if (self.loglevel >= level and self.outfile != ''):
			logmsg = "{0} [{1}]: {2}\n".format( time.strftime("%Y/%m/%d %H:%M:%S", time.localtime( time.time() ) ), os.getpid(), ustr).encode('utf-8')
			self.logfh.write(logmsg)
			self.logfh.flush()
